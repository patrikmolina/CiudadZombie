...
var Dibujante = {
  ...
  dibujarJugador: function (jugador, movX, movY) {
    if(movY === 0 && jugador.ancho<jugador.alto){
      var temp = jugador.ancho;
      jugador.ancho = jugador.alto;
      jugador.alto = temp;
    }
    else if (movX === 0 && jugador.ancho>jugador.alto){
      var temp2 = jugador.ancho;
      jugador.ancho = jugador.alto
      jugador.alto = temp2;
    }

    this.dibujarEntidad(jugador);
  },
  ...
  dibujarTexto: function(texto, color, x, y){
    var ctx = this.canvas.getContext('2d');
    ctx.fillStyle = color;
